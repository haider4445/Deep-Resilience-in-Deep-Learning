...
class DMRIFGSM(Attack):
    ...
    def __init__(self, model, targeted = 0, eps=16/255, alpha=8/255, steps=8, decay=1.0,
                 resize_rate=0.9, diversity_prob=0.5, random_start=False):
        super().__init__("DMRIFGSM", model)
        self.eps = eps
        self.steps = steps
        self.decay = decay
        self.alpha = alpha
    ...
    def forward(self, images, labels):
        ...
        adv_images = images + self.alpha*torch.randn_like(adv_images).sign()
        adv_images = torch.clamp(adv_images, min=0, max=1).detach()
